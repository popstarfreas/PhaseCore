...
class Utils {
    ...
    static autoVideoFormat(match, link, protocol, www, domain, regexDotGroup, tld, port, filepath, getParams, imageExtension, endChar) {
      //console.log(match);
      var videoSearchLength = this.videoSearch.length;
      ...
      if (videoSites.indexOf(domain) > -1 && (typeof getParams !== 'undefined' && getParams.length > 0 || domain == "youtu" && tld == ".be")) {
        for (var i = 0; i < videoSearchLength; i++) {
          output = output.replace(this.videoSearch[i], this.videoReplace[i]);
        }
